...
try:
    import redis.asyncio as redis  # dependência: redis>=4.2+ (v5+)
...
async def get_redis_client() -> "redis.Redis":
    ...
    if _redis_client is None:
        ...
        try:
            ...
            # logue e continue para que a app não quebre na import
            pass
    return _redis_client

# Ao finalizar a aplicação (opcional): fechar conexão
    ...
    async def clear_pattern(self, pattern: str) -> int:
        ...
        try:
            ...
            async for key in self.redis_client.scan_iter(match=pattern, count=1000):
                total_deleted += int(await self.redis_client.delete(key))
            logger.debug(
...
async def invalidate_tws_cache(
    ...
    try:
        ...
        if scope == "system":
            ...
            if redis_manager:
                await redis_manager.clear_pattern("tws:*")
        ...
        if scope == "jobs":
            ...
            if redis_manager:
                await redis_manager.clear_pattern("tws:jobs:*")
        ...
        if scope == "workstations":
            ...
            if redis_manager:
                await redis_manager.clear_pattern("tws:workstations:*")
