...
async def get_simplified_health_service_manager() -> (
    ...
    """
    nonlocal _health_service_manager
    ...
    async with _manager_lock:
        ...
        if _health_service_manager is None:
            logger.info("Creating global simplified health service manager")
            _health_service_manager = SimplifiedHealthServiceManager()
            logger.info("Global simplified health service manager created")
...
async def shutdown_simplified_health_service() -> None:
    ...
    This function ensures proper cleanup of the health service,
    ...
    if _health_service_manager is not None:
        try:
            await _health_service_manager.shutdown_service()
            _health_service_manager = None
            logger.info(
                "Global simplified health service manager shutdown completed"
            )
        except Exception as e:
            logger.error(
                "Error during global simplified health service manager shutdown",
                error=str(e),
            )
            raise
    else:
        logger.debug(
            "Simplified health service manager already shutdown or never initialized"
        )
