...
class HealthCheckConfig:
    ...
    alert_threshold_degraded: int = (
        1  # Number of degraded components to trigger alert
    )
    ...
    response_time_threshold_ms: int = (
        1000  # Response time threshold in milliseconds
    )

    # Memory bounds configuration
    max_history_entries: int = 1000  # Maximum number of health history entries
    history_cleanup_threshold: float = (
        0.8  # Cleanup when 80% of max entries reached
    )
    history_cleanup_batch_size: int = (
        100  # Number of entries to remove per cleanup
    )
    enable_memory_monitoring: bool = True  # Enable memory usage monitoring
    memory_usage_threshold_mb: int = (
        50  # Alert when memory usage exceeds this (MB)
    )
    history_compression_enabled: bool = (
        False  # Enable compression for old entries
    )
...
class HealthCheckError(Exception):
    ...
    def __init__(
        self, component: str, message: str, status_code: str | None = None
    ):
