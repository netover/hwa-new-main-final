...
class ErrorFactory:
    ...
    def create_error_response(
        ...
        include_stack_trace = (
            should_include_stack_trace_func() and not is_production
        )

        # In production, sanitize error messages to prevent information disclosure
        if is_production:
            builder.with_stack_trace(
                include_stack_trace and is_production is False
            )
        else:
        ...
        if isinstance(
            exception, (TWSConnectionError, EnhancedTWSConnectionError)
        ):
    ...
    def create_response(
        ...
        if exception.error_category == "EXTERNAL_SERVICE":
            ...
            return builder.build_external_service_error(
                service_name,
                user_friendly_message=sanitize_error_message(
                    user_friendly_message
                ),
                details=details,
            )
...
class TWSConnectionExceptionFactory:
    ...
    def create_response(
        builder,
        exception: Exception,
        is_production: bool,  # ErrorResponseBuilder
    ) -> BaseErrorResponse:
