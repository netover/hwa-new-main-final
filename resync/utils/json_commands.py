...
class JSONParseCommand:
    ...
    def _validate_input(self) -> None:
        ...
        if len(self.text) > MAX_TEXT_SIZE:
            raise ParsingError(
                f"Input text exceeds maximum size of {MAX_TEXT_SIZE} bytes"
            )
    ...
    def _extract_json(self) -> None:
        ...
        if len(self.json_str) > self.max_size:
            raise ParsingError(
                f"JSON content exceeds maximum size of {self.max_size} bytes"
            )
    ...
    def _validate_keys(self) -> None:
        ...
        missing_keys = [
            key for key in self.required_keys if key not in self.result
        ]
        if missing_keys:
            raise ParsingError(
