...
def log_error_response(
    ...
    if error_response.severity == ErrorSeverity.CRITICAL:
        logger.critical(
            ...
            exc_info=(
                original_exception if should_include_stack_trace() else None
            ),
        )
...
def get_error_status_code(error_category: ErrorCategory) -> int:
    ...
    return status_code_map.get(
        error_category, status.HTTP_500_INTERNAL_SERVER_ERROR
    )
...
def create_json_response_from_error(
    error_response: BaseErrorResponse,
) -> JSONResponse:
...
def register_exception_handlers(app):
    ...
    async def enhanced_resync_exception_handler(
    ...
    async def base_resync_exception_handler(
        request: Request, exc: BaseResyncException
    ):
        correlation_id = generate_correlation_id()
        error_response = create_error_response_from_exception(
            exc, request, correlation_id
