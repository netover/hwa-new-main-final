...
with open('analysis_reports/pyright_final_missing_imports.json', 'r') as f:
    data = json.load(f)
...
for i, imp in enumerate(missing_imports[:10], 1):
    print(f'{i}. {imp["file"]}:{imp["line"]} - {imp["message"]}')
