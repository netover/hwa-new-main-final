...
class CacheMiddleware(BaseHTTPMiddleware):
    ...
    def _should_not_cache_response(self, response: Response) -> bool:
        ...
        if hasattr(response, 'streaming') and getattr(response, 'streaming', False):
            return True
    ...
    def _add_cache_headers(self, request: Request, response: Response) -> None:
        ...
        if self.config["last_modified_enabled"]:
            if not hasattr(response, 'last_modified') or not getattr(response, 'last_modified', None):
                ...
                if hasattr(response, 'last_modified'):
                    setattr(response, 'last_modified', last_modified)
