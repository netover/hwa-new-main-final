#!/usr/bin/env python3
"""
Script focado para analisar e corrigir apenas os erros mais críticos.
"""
...
class FocusedFixer:
    def __init__(self, reports_dir: str = "analysis_reports"):
        ...
        self.max_batches = 5  # Limitar a 5 lotes (100 erros)
    ...
    def get_top_issues(self, all_issues: List[Issue], limit: int = 100) -> List[Issue]:
        """Obtém as issues mais críticas, limitando ao número especificado."""
        # Filtra apenas issues CRITICAL e HIGH
        ...
        sorted_issues = sorted(critical_high_issues, key=lambda x: (
            -self.severity_weights[x.severity],
            x.file,
            x.line
        ))
        
        # Retorna o número limitado de issues
    ...
    def generate_focused_report(self, issues: List[Issue], output_file: str = None) -> str:
        """Gera um relatório focado das correções aplicadas."""
        if not output_file:
            output_file = self.reports_dir / "focused_fix_report.txt"
        
        report_lines = []
        report_lines.append("# RELATÓRIO FOCADO DE CORREÇÕES")
        report_lines.append(f"Total de Issues Analisadas: {len(issues)}")
        report_lines.append("")
        ...
        report_lines.append("## DETALHES DAS CORREÇÕES")
...
if __name__ == "__main__":
    ...
    fixer = FocusedFixer(current_dir)
    
    # Carrega todas as issues
    all_issues = fixer.load_all_issues()
    
    # Obtém as 100 issues mais críticas
    top_issues = fixer.get_top_issues(all_issues, 100)
    
    # Aplica as correções
    fixes_count = fixer.apply_fixes(top_issues)
    
    # Gera o relatório focado
    report = fixer.generate_focused_report(top_issues)
    
    print(f"Análise focada concluída para {len(top_issues)} issues.")
    print(f"Correções aplicadas: {fixes_count}")
    print(f"Relatório salvo em: {os.path.join(current_dir, 'focused_fix_report.txt')}")
