...
class TestTypeAnnotationsAndDataValidation:
    ...
    def test_base_settings_environment_validation(self):
        """Test environment validation in BaseSettings."""
        # Valid environments
        settings = BaseSettings(environment=Environment.DEVELOPMENT)
        assert settings.environment == "development"

        settings = BaseSettings(environment=Environment.PRODUCTION)
        assert settings.environment == "production"

        # Invalid environment
        with pytest.raises(ValidationError):
            BaseSettings(environment="invalid_env")
...
class TestErrorHandling:
    ...
    def test_handle_error_function(self):
        """Test the improved handle_error function from endpoints module."""
