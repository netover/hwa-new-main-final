...
class TestCircuitBreakerStateTransitions:
    ...
    async def test_redis_circuit_breaker_opening(self):
        ...
        try:
            await redis_breaker.call_async(failing_redis_operation)
            raise AssertionError("Expected CircuitBreakerError to be raised")
    ...
    async def test_tws_circuit_breaker_excludes_authentication_errors(self):
        ...
        for _i in range(10):
            try:
                ...
                raise AssertionError("Expected AuthenticationError to be raised")
...
class TestCircuitBreakerStatusManagement:
    ...
    async def test_reset_all_circuit_breakers(self):
        ...
        # Open the Redis circuit breaker by triggering failures
        for _ in range(3):
            try:
                await redis_breaker.call_async(failing_redis_operation)
            except RedisConnectionError:
                # Expected failure; continue to open the circuit
                pass

        # Verify circuit is OPEN
        assert redis_breaker.current_state.__class__.__name__ == "CircuitOpenState"

        # Reset all circuit breakers (synchronous)
        reset_all_circuit_breakers()

        # Verify circuits are CLOSED and counters reset
        assert redis_breaker.current_state.__class__.__name__ == "CircuitClosedState"
