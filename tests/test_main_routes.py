...
class TestMiddleware:
    ...
    def test_before_request_generates_request_id(self, client):
        ...
        from routes.main import app, before_request

        # Mock request object
        mock_request = MagicMock()
        mock_request.headers.get.return_value = None

        with app.test_request_context("/"), patch("routes.main.request", mock_request):
            before_request()
    ...
    def test_after_request_logs_completion(self, client):
        ...
        with app.test_request_context("/"):
            ...
            with patch("routes.main.request", mock_request), patch(
                "routes.main.log_request_end"
            ) as mock_log:
...
class TestLoggingContext:
    ...
    def test_login_route_uses_logging_context(self, client):
        ...
        with app.test_request_context("/login"):
            ...
            with patch("routes.main.LoggingContext") as mock_context, patch(
                "routes.main.log_info"
            ) as mock_log:
                login()
    ...
    def test_chat_route_uses_logging_context(self, client):
        ...
        with app.test_request_context("/api/chat", method="POST"):
            ...
            with patch("routes.main.request", mock_request), patch(
                "routes.main.LoggingContext"
            ) as mock_context, patch("routes.main.log_info") as mock_log, patch(
                "routes.main.abort"
            ):  # Prevent actual abort
